        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <h1 className="text-lg sm:text-2xl font-bold text-red-900">Новинки</h1>
            </div>
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="sm:hidden p-2 text-slate-700">
          ...
          {menuOpen && (
            <div className="sm:hidden mt-4 pb-2 space-y-2">
              <RequestForm />
              {isAdmin ? (
                <Link href="/admin" className="block w-full px-4 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition text-center">
                  ⚙️ Админ панель
                </Link>
              ) : (
                <Link href="/bookmarks" className="block w-full px-4 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition text-center">
                  🔖 Закладки
                </Link>
              )}
              <button onClick={handleLogout} className="w-full px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition">
                Выйти
              </button>
            </div>
          )}
